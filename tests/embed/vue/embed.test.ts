...
describe('Embedded', () => {
	...
	// Enable when https://github.com/prettier/prettier/issues/9049 is resolved
	test.skip('should format when embedded in vue empty template', () => {
		const expected: string = readFileSync(resolve(__dirname, 'formatted-empty-template.vue'), 'utf8');
		const code: string = readFileSync(resolve(__dirname, 'unformatted-empty-template.vue'), 'utf8');
		const actual: string = format(code, { parser: 'vue', plugins: [plugin] });
